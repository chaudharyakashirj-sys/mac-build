      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="glass rounded-2xl p-6 sm:p-8 flex flex-col items-center shadow-sm relative overflow-hidden group">
            ...
            {/* Session Timers */}
            <div className="mt-8 w-full grid grid-cols-2 sm:grid-cols-5 gap-3 sm:gap-4 relative z-10">
              ...
              <div className="bg-white/80 backdrop-blur-sm p-4 rounded-xl shadow-sm border border-orange-100 text-center transition-all hover:shadow-md hover:scale-[1.02]">
                <div className="text-[10px] sm:text-xs text-slate-500 font-bold uppercase tracking-wider">Break</div>
                <div className="text-xl sm:text-2xl font-black text-orange-600 mt-1">{formatHMS(getTodayTotalBreakSeconds())}</div>
              </div>
            ...
            {isWorking && !isMeetingMode && !isBreakMode && idleSeconds >= (AUTO_OFF_IDLE_SECONDS - 60) && idleSeconds < AUTO_OFF_IDLE_SECONDS && (
              <div className="mt-6 w-full">
                <div className="p-4 bg-red-50 border-2 border-red-300 rounded-lg text-red-800">
                  ...
                  <div className="text-xs mt-3 text-red-700">
                    💡 Move your mouse or press a key to stay online, or click "Enter Meeting Mode" / "Enter Break Mode" if needed.
                  </div>
            ...
            <div className="mt-6 w-full text-center">
              ...
              {isBreakMode && (
                <div className="mb-3 p-3 bg-orange-50 rounded border border-orange-100 text-orange-700 font-semibold">Break Mode Active — Idle Not Tracked</div>
              )}
              <div className="flex flex-wrap justify-center gap-3">
                ...
                <button
                  onClick={async () => {
                    ...
                    console.log(`🟠 [Break Mode Toggle] ${next ? 'ENTERING' : 'EXITING'} break mode`);
                    setIsBreakMode(next);
                    breakModeRef.current = next;
                    autoStoppedRef.current = false;
                    localStorage.setItem('breakMode', next ? 'true' : 'false');
                    if (isElectronRef.current) {
                      const electron = (window as any).electron;
                      await electron.setBreakMode(next);
                      console.log(`✅ [Break Mode] Electron notified: ${next ? 'ON' : 'OFF'}`);
                    }
                    if (next) {
                      breakSecondsRef.current = 0;
                      breakCountRef.current = (breakCountRef.current || 0) + 1;
                      setBreakSeconds(0);
                      lastActivityRef.current = Date.now();
                      console.log(`✅ [Break Mode] ENTERED - Idle timer reset.`);
                    } else {
                      lastActivityRef.current = Date.now();
                      console.log(`✅ [Break Mode] EXITED - Idle timer reset.`);
                    }
                  }}
                  className={`px-6 py-3 rounded-md font-medium ${isBreakMode ? 'bg-orange-500 text-white' : 'bg-orange-100 text-orange-700'}`}
                >
                  {isBreakMode ? 'Exit Break' : 'Enter Break'}
                </button>
              </div>
            </div>
          </div>
